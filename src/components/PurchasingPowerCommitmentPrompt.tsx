  ...
  return (
    ...
    >
      <div className="panel relative w-full max-w-lg overflow-hidden rounded-lg p-6 shadow-2xl sm:p-7">
        ...
        <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-md border border-[rgba(240,163,111,0.24)] bg-black/18 p-4">
          <input
            className="mt-1 h-4 w-4 accent-[#f0a36f]"
            type="checkbox"
            checked={isChecked}
            onChange={(event) => setIsChecked(event.target.checked)}
          />
          <span className="text-sm leading-6 text-[#d9ccbd]">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <p className="rounded-md border border-[rgba(239,230,218,0.14)] bg-black/18 p-3 text-xs leading-5 text-[#d9ccbd]">
