...
import { calculateScenario, formatBTC } from "./calculations";
import {
  formatCurrency,
...
export type WeeklyReportEmailInput = {
  ...
  btcPrices?: Partial<Record<CurrencyCode, BTCPriceResult>>;
...
export function buildWeeklyReportEmail(input: WeeklyReportEmailInput) {
  ...
  const rows = input.scenarios
    ...
    .map(({ scenario, baselineItemCostBTC }) => {
      ...
      const btcPrice =
        input.btcPrices?.[currencyCode] ?? input.btcPrice ?? input.btcPrices?.USD;
      const currentScenario = {
        ...scenario,
        currencyCode,
        currentBTCPriceUSD: btcPrice?.price ?? scenario.currentBTCPriceUSD,
      };
      ...
      return { itemName: scenario.itemName, currentCostBTC, direction, currencyCode };
    });
  ...
  const scenarioMarkup = reportRows
    ...
    .join("");
  const prices = Object.values(input.btcPrices ?? {}).filter(
    (price): price is BTCPriceResult => Boolean(price),
  );
  if (prices.length === 0 && input.btcPrice) prices.push(input.btcPrice);
  const priceContext = prices.length
    ? prices
        .map(
          (price) =>
            `${formatCurrency(price.price, price.currencyCode)} BTC (${price.status === "live" ? "live reference" : "editable fallback"})`,
        )
        .join("; ")
    : "saved scenario reference prices";
